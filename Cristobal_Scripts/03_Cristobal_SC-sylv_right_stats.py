...
for ndim in ['32','75','256']:
    ...
    model_path = '/neurospin/dico/cmendoza/Runs/Jeanzay/01_betavae_sulci_crops/2025-06-12/SC-sylv_right_UKB_'+ndim+'/'
    ...
    Reconstruction_error = pd.read_csv(model_path+'Reconstruction_error_SC-sylv_right_UKB_'+ndim+'_.csv')
    ...
    plt.title('Reconstruction error SC-sylv right UKB ndim:'+ndim)
    plt.legend()
    ...
    Embeddings = pd.read_csv(model_path+'Embeddings_SC-sylv_right_UKB_ndim_'+ndim+'.csv')
    columns = ['ID']
    for i in range(0,int(ndim)):
    ...
    labels_0 = [0]*207
    Embeddings_test.insert(loc=int(ndim)+1, column='labels', value=labels_0)
    print('Embedding test',Embeddings_test)

    labels_1 = [1]*207
    Embeddings_interrupted.insert(loc=int(ndim)+1, column='labels', value=labels_1)
    ...
    plt.xlabel('FPR')
    plt.ylabel('TPR')
    plt.title('ROC Stratified K-Cross Fold Validation, SC-sylv right UKB ndim '+ndim)
